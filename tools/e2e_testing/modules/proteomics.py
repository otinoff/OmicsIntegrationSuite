"""
Proteomics Module E2E Test
End-to-end testing for proteomics module using browser automation and server validation
"""
...
class ProteomicsTest:
    """End-to-end test for Proteomics module"""

    def __init__(self, headless: bool = False):
        """
        Initialize Proteomics test

        Args:
            headless: Run browser in headless mode
        """
        self.headless = headless
        self.module_name = "proteomics"
        self.results = {}

    def run_test(self, file_name: str, timeout: int = 600) -> Dict[str, bool]:
        """
        Run complete E2E test for proteomics module

        Args:
            file_name: Name of the mzML file to test
        ...
        """
        print("\n" + "=" * 70)
        print(f"PROTEOMICS E2E TEST: {file_name}")
        print("=" * 70)
    ...
    def _run_browser_test(self, file_name: str, timeout: int) -> bool:
        ...
        try:
            with BrowserHelper(headless=self.headless) as browser:
                ...
                # Give server time to write files (JSON export needs extra time)
                print("⏳ Waiting for server to write result files (10s)...")
                time.sleep(10)
    ...
    def _run_validation(self, file_name: str) -> Dict[str, bool]:
        ...
        # Check marker file
        marker_path = "/tmp/proteomics_processing_started.txt"
        results["marker_file"] = validator.validate_marker_file(marker_path)

        # Check JSON export
        json_path = "/var/OmicsIntegrationSuite/data/validation_json/proteomics_latest_results.json"
        results["json_export"] = validator.validate_json_export(json_path, "proteomics")

        # Check uploaded file still exists (proteomics processes files in-place, no copy to processed/)
        uploaded_file = f"/var/OmicsIntegrationSuite/data/proteomics/uploaded_files/{file_name}"
        results["uploaded_file_exists"] = validator.check_file_exists(
            uploaded_file,
...
if __name__ == "__main__":
    """Run proteomics test when executed directly"""
    import sys
    ...
    test = ProteomicsTest(headless=headless)
