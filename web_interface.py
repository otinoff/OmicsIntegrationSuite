...
if module == "🏠 Главная":
    ...
    # Импорт UI компонента микроРНК
    try:
        from ui_components.mirna_module import render_mirna_module
        render_mirna_module()
    except ImportError as e:
        st.error(f"❌ Ошибка загрузки модуля микроРНК: {e}")
        st.header("🔬 Модуль обработки данных микроРНК")
        st.info("Модуль микроРНК временно недоступен. Проверьте установку компонентов.")
    except Exception as e:
        st.error(f"❌ Ошибка выполнения модуля микроРНК: {e}")
        st.header("🔬 Модуль обработки данных микроРНК")
        st.info("Произошла ошибка при загрузке модуля. Проверьте логи для деталей.")

elif module == "🦠 Протеомика":
    # Импорт UI компонента протеомики
    try:
        from ui_components.proteomics_module import render_proteomics_module
        render_proteomics_module()
    except ImportError as e:
        st.error(f"❌ Ошибка загрузки модуля протеомики: {e}")
        st.header("🧬 Модуль обработки протеомных данных")
        st.info("Модуль протеомики временно недоступен. Проверьте установку компонентов.")
    except Exception as e:
        st.error(f"❌ Ошибка выполнения модуля протеомики: {e}")
        st.header("🧬 Модуль обработки протеомных данных")
        st.info("Произошла ошибка при загрузке модуля. Проверьте логи для деталей.")
